...
module.exports = {
  ...
  obtenerProducto: async ({ _id }: any) => {
    try {
      const producto = await model?.traerProducto(_id)
      return producto
    }
    catch (error) {
  ...
  crearProducto: async ({ nombre, descripcion, codigo, foto, precio, stock }: any) => {
    try {
      let producto = { nombre, descripcion, codigo, foto, precio, stock }
      const productoCreado = await model?.agregarProducto(producto)

      return productoCreado
    }
    catch (error) {
      console.log('error crearProducto', error)
    }
  },

  actualizarProducto: async ({ _id, update }: any) => {
    try {
      const productoActualizado: any = await model?.actualizarProducto(_id, update)
  ...
  borrarProducto: async ({ _id }: any) => {
    try {
      let productoBorrada = await await model?.eliminarProducto(_id)
